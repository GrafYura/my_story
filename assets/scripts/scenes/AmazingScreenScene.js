class AmazingScreenScene extends Scene{
	constructor(){
		super("AmazingScreen");
	}
	addPaul(){
	...
	paulSpeak(){
		this.delayedCall(()=>{this.paul.speak();
			this.showSpeach('speach_Paul2');
		},this.animDuration)
		this.delayedCall(()=>{this.hideSpeach()},3000);
		this.delayedCall(()=>{this.scene.start("End", {lexi:this.lexi.conf, paul:this.paul.conf, button:'playNow'})},3500)
	}
}